...
def get_command_from_gpt(prompt):
    # Load the API key and Organization ID from environment variables
    api_key = os.getenv('openAIKey')
    organization_id = os.getenv('openAIOrgId')

    # Check API key and Organization ID
    if not api_key or not organization_id:
        print("API key or Organization ID not found in environment variables.")
        sys.exit(1)
